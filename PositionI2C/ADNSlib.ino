...
byte adns_read_reg(byte reg_addr){
  ...
  delayMicroseconds(1); // tSCLK-ncsADNS for read operation is 120ns
  adns_com_end();
  delayMicroseconds(19); //  tSRW/tSRR (=20us) minus tSCLK-ncsADNS
...
void adns_write_reg(byte reg_addr, byte data){
  ...
  delayMicroseconds(20); // tSCLK-ncsADNS for write operation
  adns_com_end();
  delayMicroseconds(100); // tSWW/tSWR (=120us) minus tSCLK-ncsADNS. Could be shortened, but is looks like a safe lower bound
}
...
void performStartup(void){
  ...
  delay(10);
  adns_write_reg(REG_Configuration_I, 0x07); //1400 cpi | hex x 200 = cpi
  delay(10);
...
void UpdatePointer(void){                                            //Read values from laser motion sensor
    ...
    dx= (xydat[1] << 8 | xydat[0]);                                  //Save values in variable
    dy= -1*(xydat[3] << 8 | xydat[2]);                               //*-1 because direction is wrong otherwise
}
